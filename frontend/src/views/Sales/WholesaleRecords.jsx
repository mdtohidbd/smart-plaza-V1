...
const WholesaleRecords = () => {
  ...
  const { data: wholesaleRecords, isLoading: wholesaleRecordsLoading, refetch } = useQuery(
    'wholesale-records',
    async () => {
      const response = await api.get('/api/sales');
      return response.data.data.filter(sale => sale.type === 'wholesale');
    },
    {
  ...
  const filteredRecords = useMemo(() => {
    if (!wholesaleRecords) return [];
    if (!searchTerm) return wholesaleRecords;

    const term = searchTerm.toLowerCase();
    const filtered = wholesaleRecords.filter(record => 
      record.invoiceNumber.toLowerCase().includes(term) ||
      (record.customer?.contactName && record.customer.contactName.toLowerCase().includes(term)) ||
      (record.total && record.total.toString().includes(term)) ||
      (record.paidAmount && record.paidAmount.toString().includes(term)) ||
      (record.dueAmount && record.dueAmount.toString().includes(term)) ||
      ...
      <Grid container spacing={1.5}>
            ...
            <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 2, flexWrap: 'wrap', gap: 1.5 }}>
              <Box>
                ...
                <Typography variant="body2" sx={{ color: 'text.secondary', fontFamily: '"Outfit", sans-serif' }}>
                  View and manage recent wholesale sales transactions.
                </Typography>
              </Box>
              <ExportButtons
                data={filteredRecords || []}
                columns={columns}
                filename="wholesale_sales_records"
            ...
            <Grid container spacing={1.5} alignItems="center">
              <Grid item xs={12} md={6}>
                <TextField
                  ...
                  variant="outlined"
                  size="small"
                  placeholder="Search wholesale records..."
                  value={searchTerm}
        ...
        <Grid item xs={12}>
          <Card elevation={0}
            ...
            <CardHeader
              title="Wholesale Records"
              subheader="Recent wholesale sales"
              sx={{
...
export default WholesaleRecords;
