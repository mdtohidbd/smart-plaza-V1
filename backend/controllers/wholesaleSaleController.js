...
// @desc    Create wholesale sale
// @route   POST /api/sales (dispatched from routes based on type='wholesale' or default)
// @access  Private
const createWholesaleSale = asyncHandler(async (req, res) => {
  let {
    invoiceNumber,
    customer,
    items,
  ...
  const saleData = {
    ...
    type: type || 'wholesale', // Default to wholesale
  ...
  try {
    ...
    if (customerDoc) {
      ...
      await sendSaleConfirmationSMS(customerDoc, saleWithProducts, saleWithProducts.type || 'wholesale');
    }
  } catch (smsError) {
    console.error('Failed to send SMS for wholesale sale:', smsError.message);
  }
  ...
  try {
    ...
    console.error('Failed to sync wholesale sale ledger entries:', ledgerError.message);
  }
